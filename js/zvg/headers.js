define(["exports", "core/utils", "core/pubsubhub", "w3c/templates/cgbg-sotd", "w3c/templates/sotd", "w3c/templates/cgbg-headers", "w3c/templates/headers"], function (_exports, _utils, _pubsubhub, _cgbgSotd, _sotd, _cgbgHeaders, _headers) {
  "use strict";
  ...
  function validateDateAndRecover(conf, prop) {
    let fallbackDate = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : new Date();
  ...
  function run(conf) {
    ...
    if (conf.isRegular) conf.thisVersion = "https://developer.zorgeloosvastgoed.nl/informatiemodel/" + (0, _utils.concatDate)(conf.publishDate);
    if (conf.specStatus === "ED") conf.thisVersion = conf.edDraftURI;
    if (conf.isRegular) conf.latestVersion = "https://developer.zorgeloosvastgoed.nl/informatiemodel"; //"https://www.w3.org/" + publishSpace + "/" + conf.shortName + "/";
    ...
    if (conf.previousPublishDate) {
      ...
      if (conf.isTagFinding) {
        ...
        conf.prevVersion = "https://developer.zorgeloosvastgoed.nl/informatiemodel/" + (0, _utils.concatDate)(conf.previousPublishDate);
      }
    } else {
    ...
    conf.prependW3C = false; //!conf.isUnofficial;
    ...
    if (conf.isIGNote && !conf.charterDisclosureURI) (0, _pubsubhub.pub)("error", "IG-NOTEs must link to charter's disclosure section using `charterDisclosureURI`."); // ensure subjectPrefix is encoded before using template
  ...
  function populateSoTD(conf, sotd) {
    ...
    return (conf.isCGBG ? _cgbgSotd.default : _sotd.default)(conf);
  }
});
